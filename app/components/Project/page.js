...
const Project = () => {
  ...
  return (
    <section id="project" className="bg-zinc-900 py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 className="text-4xl font-extrabold text-blue-500 text-center mb-8 font-Lato">
          Projects
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {projects.map((project, index) => (
            ...
            >
              <h3 className="text-xl text-blue-500 font-bold mb-4 font-Ubuntu">
                {project.title}
              </h3>
              <img
                src={project.image}
                alt={project.title}
                className="w-full lg:w-3/2 h-48 object-cover rounded"
              />
              {/* Render the image */}
              <p className=" text-gray-500 mt-2 mb-4 font-Ubuntu">
                {project.description}
              </p>
              <a
                ...
                className="bg-blue-500 text-white font-Ubuntu py-2 px-4 rounded hover:bg-blue-700 focus:outline-none focus:shadow-outline"
              >
                GitHub Repository
              </a>
            </div>
