...
const Project = () => {
  ...
  <h2 className="text-3xl font-extrabold text-blue-500 text-center mb-8 font-sans">Projects</h2>
  <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
    {projects.map((project, index) => (
      <div key={index} className=" rounded-lg  p-6 hover:transform hover:scale-105 hover:bg-blue-200 transition duration-200 ease-in-out">
        <h3 className="text-xl text-orange-500 font-bold mb-4 font-sans">{project.title}</h3>
        <p className=" text-gray-500 mb-4 font-sans">{project.description}</p>
        <a href={project.githubLink} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">GitHub Repository</a>
      </div>
    ))}
  </div>
</div>
